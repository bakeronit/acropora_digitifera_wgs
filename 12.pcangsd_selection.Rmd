...
```{r}
pbsdata <- read_tsv("data/hpc/selection2/pbs/plink2.pbs_scaff.tsv",col_types = cols(),col_names = c("chr","pos","PBS_IN","PBS_NO","PBS_SO")) %>% 
...
mh_high <- manhattan_data %>% 
  filter(PBS>0.5)

mh_low <- manhattan_data %>% 
  filter(PBS<0.5) %>% 
  slice_sample(n=nrow(mh_high))

rbind(mh_low,mh_high) %>% 
  ggplot() + 
