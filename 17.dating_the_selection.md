...
mutation rate. A range is given simply by taking the distribution of the
trace in the last 10000 MCMC steps. A better estimate should be obtained
by running multiple chains to ensure convergence and making parameter
estimates from converged chains only.

One of the main things that can be seen here is that very few loci pass
the QC checks for use with startmrca. This severely limits the analysis,
however we are able to see dating estimates for one locus involved in
the enriched go term, hemidesmosome assembly. This appears to be a
relatively recent sweep (\~6kya) which approximately matches the time
when sea levels reached similar heights to what they are today.

If we were to look at time estimates including all loci (not filtered
for QC) we would see that they span a very wide range of times with many
in the range &gt;20kya. Aside from issues with identification of a
star-like selection signature this also raises the issue of what effect
a large demographic change would have on these putative selection
signals. The process of QC seems to have naturally enriched for some of
the more recent selective sweeps, and the sweep associated with
hemidesmosome is recent enough that it is from a time period where
changes in Ne were much less dramatic.

<img src="17.dating_the_selection_files/figure-gfm/unnamed-chunk-13-1.png" width="672" />
