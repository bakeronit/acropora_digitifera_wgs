...
3.  Pick the `position` as the max_xpehh_pos if xpehh is significant,
    otherwise max_ihs_pos
...
EHH that extends over a large (>100kb) region. However we can also see
that there are some inshore haplotypes with allele 0 that furcate early
(ie probably don’t have the selected haplotype).
