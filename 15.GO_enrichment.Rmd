...
bar_go1 <- topgo_results_all %>% 
  group_by(GO.ID) %>% 
  dplyr::summarise(max_sig=max(Significant),minwf=min(WeightFisher)) %>% 
  filter(max_sig>0) %>% 
  filter(minwf<0.001) %>% 
  pull(GO.ID) %>% 
  unique()
...
bar_go <- topgo_results_all %>% 
  ...
  mutate(n_regions = map2_int(GO.ID,pop,n_regions_per_term)) %>% 
  group_by(GO.ID) %>% 
  dplyr::summarise(max_regions = max(n_regions)) %>% 
...
We found a total of `r pg_go %>% filter(classic<0.001) %>% filter(n_regions>2) %>% pull(GO.ID) %>% unique() %>% length()` GO terms enriched (p<0.001) across all three of the locations.  These are dplyr::summarised in Figure 1.  
...
gene_table_for_term <- function(goterm){
  ...
  ungroup() %>% 
    ...
    dplyr::select(enriched_go,region_id,pop,genes,uniprot_id,genename,protein,frac,stats) %>% 
    na.omit()
}
...
allgo_genes <- map_dfr(go_terms_in_plot,gene_table_for_term) %>% 
  ...
  dplyr::select(enriched_go,TERM,region_id,pop,genes,uniprot_id,genename,protein,frac,stats)
