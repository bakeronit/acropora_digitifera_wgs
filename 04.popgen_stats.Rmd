...
We counted the number of SNPs specific to each population. These are variants that are only exist in one population and are homozygous in other samples.
...
If the threshold is set lower to 0.9 far more SNPs (95) are identified but all are in the inshore population and all are concentrated in just four loci. Two of these fall within putative sweeps and two fall just outside them within 50kb. 
...
het_data <- read_tsv("data/hpc/popgen/plink2.het") %>% 
  ...
  mutate(location= case_when(
      grepl("^AI",sample) ~ "Inshore",
      grepl("^BR",sample) ~ "Inshore",
      grepl("^AR",sample) ~ "North Offshore",
      grepl("^RS",sample) ~ "South Offshore"
          )) %>% 
...
het_data %>% 
  ...
  xlab("") + ylab("Inbreeding coeffient, F")
```


```{r}
ggsave("figures/inbreeding.png",width = 3,height = 3)
```
...
**Figure:** The left boxplot shows the values of $\theta_{\pi}$ and the right plot displays the Tajima's D values in three populations.
