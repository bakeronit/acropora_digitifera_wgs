...
ic_afp <- ic_af %>% mutate(correct=code1==code2) %>% 
  ...
  ggplot(aes(x=group,y=accuracy,group=group)) + geom_point() + labs(x="Minor allele frequency",y="Accuracy") + theme(axis.text.x = element_text(angle=90))

plot_grid(icp,ic_afp,labels = c("A","B"))
ggsave("figures/imputation_accuracy.png")
```
