...
ic_af %>% mutate(correct=code1==code2) %>% 
  group_by(group=cut(af,breaks=seq(0,max(af),length.out=10))) %>% summarise(accuracy=sum(correct)/n()) %>% ggplot(aes(x=group,y=accuracy,group=group)) + geom_point() + labs(x="Minor allele frequency",y="Accuracy") + theme(axis.text.x = element_text(angle=90))
