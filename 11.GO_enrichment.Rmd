...
if ( !file.exists("cache/topgo_results_all.rds") ){
  ...
  write_rds(topgo_results_all_gene,"cache/topgo_results_all.rds")
  write_tsv(topgo_results_all_gene,"cache/topgo_results_all.tsv") # This table should be in Supp Info
} else {
...
barplot <- pd %>% 
  ...
  mutate(is_enriched = ifelse(p.region<0.001, 1, 0.9)) %>% 
  ggplot(aes(x=-log10(p.region),y=GO.ID,fill=ontology)) + 
...
#  ggstance::geom_colh(aes(alpha=is_enriched)) +
  ...
  geom_text(aes(label=text_label), hjust=-0.5,size=2) +
  theme_bw() + 
...
geom_tiplab(aes(label = term_label),align = TRUE, size=3) + 
  xlim(0,10) 

bpt <- barplot %>% insert_left(tree, width = 1.2)
