...
supp_table4 <- candidate_regions_genes_ehh %>% 
  ...
  rename(Scaffold=chr,Start=start, End=end,Population=pop,"Gene names in this window"=protein_names,"Gene ids in this window"=gene_ids,"Fraction with z-score > 2"=frac) %>% 
  ungroup()
...
supp_table5 <- candidate_regions_genes_ehh %>% 
  separate_rows(genes,sep=";") %>% 
  left_join(uniprot_gene_annot, by = c("genes"="geneid") ) %>%
  rename(Scaffold=chr,Start=start, End=end,Population=pop, "Gene ID"=genes,"Fraction with z-score > 2"=frac, "Protein name"=protein,"go_swissprot"=go) %>% 
  select(-genename) 
