...
supp_table9 <- candidate_regions_genes_ehh %>% 
  ...
  rename("Start"=start, "End"=end,"Population"=pop,"Genes in this window"=protein_names,"Fraction with z-score > 2"=frac) %>% 
  ungroup()

write_tsv(supp_table9,"cache/supp_table9.tsv") # For upload to google sheets  
...
candidate_regions_genes_ehh %>% 
  ...
  geom_jitter(aes(x=pop,y=frac,color=pop),alpha=0.5) + 
...
candidate_regions_genes_ehh %>% 
  ggplot() + 
  geom_density(aes(x=frac,color=pop))
```
