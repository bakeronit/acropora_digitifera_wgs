...
#   xlim(-0.2,1) + xlab("Population Branch Statistic") + ylab("") +
...
pbs_thresholds <- read_rds("cache/pbs_thresholds.rds")
...
mh_high <- pbs_manhattan_data %>% 
  filter(PBS>0.2)

mh_low <- pbs_manhattan_data %>% 
  filter(PBS<0.2) %>% 
  slice_sample(n=nrow(mh_high))


mh_plot <- rbind(mh_low,mh_high) %>% 
  filter(PBS>0) %>% 
  ggplot() + 
