...
#  filter(!is.na(dated_locus_number)) %>% 
  filter(dated_locus_number %in% c("1","8","10","37")) %>% 
#  filter(PBS>0.3) %>% 
  na.omit()
...
hldata <- to_label$locus %>% purrr::map_dfr(make_hldata)

hldata %>% 
    ggplot() + 
      geom_segment(aes(x=MIN/1e6,xend=MAX/1e6,y=reorder(rn,length),yend=reorder(rn,length),color=name)) +
    theme_pubr() + 
