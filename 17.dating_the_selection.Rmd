...
```{r}
sweep_regions <- read_rds("data/r_essentials/candidate_regions_genes_ehh.rds") %>% 
...
if (!file.exists("data/r_essentials/17_sweep_regions.rds")){
  ...
  write_rds(sweeps_gs4,"data/r_essentials/17_sweep_regions.rds")
} else {
  sweeps_gs4 <- read_rds("data/r_essentials/17_sweep_regions.rds")
}
```
...
manual_sweeps <- sweeps_gs4 %>% 
  ...
  filter(qc_pass==1) %>% 
  dplyr::select(-manual_allele,-note,-qc_pass) %>% 
...
read_chain <- function(path){
  ...
  tmp$t.chain %>% 
    as.data.frame() %>% 
    mutate(p = path) %>% 
    extract(p,into=c("locus","rep"),regex="(B.*)_([1-3]).rds") %>% 
    filter(V4==1) %>% 
    slice_sample(n=1000)
}
if ( !file.exists("data/r_essentials/17_tmrca_data.rds")){
  tmrca_data <- list.files("data/hpc/startmrca/results/","*.rds", full.names = TRUE) %>% 
    map_dfr(read_chain) %>% 
    filter(locus %in% manual_sweeps$sweep_id)
...
to_mcmc_list <- function(locus_data){
  locus_data %>% select(rep,time=V1) %>% group_by(rep) %>% group_split(.keep = F) %>% map(mcmc) %>% mcmc.list()
}
...
sweep_regions_annot <- sweep_regions %>% 
  separate_rows(genes,sep=";") %>% 
  left_join(uniprot_gene_annot, by = c("genes"="geneid") ) %>% 
  separate_rows(go,sep=";") %>% 
  mutate(go = str_trim(go))

#sweep_regions_annot %>% write_tsv("cache/sweep_regions_annot.tsv")
...
sweep_regions_of_interest <- sweep_regions_annot %>% 
...
#  filter(locus %in% converged) %>% 
  ...
  mutate(n=row_number()) %>% 
  slice_tail(n=500) %>% 
  mutate(tmrca_mean = mean(V1*5), tmrca_sd = sqrt(var(V1*5))) %>% 
  ungroup() %>% 
...
protein_names <- sweep_regions_annot %>% 
  group_by(sweep_id) %>% 
  summarise(protein_names = paste(unique(protein), collapse = ";"))

# Export to Supp table 7
supp_table7 <- pd %>% 
  filter(qc_pass==1) %>%
  group_by(locus) %>% 
  slice_sample(n=1) %>% 
  ...
  select(label,locus,chr,start,end,pop,genes,tmrca_mean,tmrca_sd) %>% 
  left_join(protein_names,by=c("locus"="sweep_id"))

write_tsv(supp_table7,"cache/supp_table7.tsv")
...
to_label <- as.data.frame(dated_loci) %>% 
  ...
  filter(dated_locus_number %in% c(1,7,18,25)) #%>% 
#  mutate(dated_locus_number=as.character(dated_locus_number))

if (!file.exists("data/r_essentials/17_furcation_data.rds")){
  furcation_data <- to_label %>% pull(locus) %>% purrr::map(analyse_haps)
  names(furcation_data) <- to_label$locus
  write_rds(furcation_data,"data/r_essentials/17_furcation_data.rds")
} else {
  furcation_data <- read_rds("data/r_essentials/17_furcation_data.rds")
}
...
```{r}
knitr::include_graphics("figures/sweep_dates_wide.png")
```
