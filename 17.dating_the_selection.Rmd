...
sweep_regions %>% 
  ...
  select(sweep_id,position,allele,chrom,start,end,pop) %>% 
...
```{r}
manual_sweeps <- readxl::read_excel("data/hpc/startmrca/sweep_regions.xlsx") %>% 
...
Preliminary results are show below for all loci passing QC.  The dates shown are based on an assumption that the recombination rate equals the mutation rate.  A range is given simply by taking the distribution of the trace in the last 10000 MCMC steps. A better estimate should be obtained by running multiple chains to ensure convergence and making parameter estimates from converged chains only.

One of the main things that can be seen here is that very few loci pass the QC checks for use with startmrca.  This severely limits the analysis, however we are able to see dating estimates for one locus involved in the enriched go term, hemidesmosome assembly.  This appears to be a relatively recent sweep (~6kya) which approximately matches the time when sea levels reached similar heights to what they are today.

If we were to look at time estimates including all loci (not filtered for QC) we would see that they span a very wide range of times with many in the range >20kya.  Aside from issues with identification of a star-like selection signature this also raises the issue of what effect a large demographic change would have on these putative selection signals.  The process of QC seems to have naturally enriched for some of the more recent selective sweeps, and the sweep associated with hemidesmosome is recent enough that it is from a time period where changes in Ne were much less dramatic.
...
go_terms_signif <- list("hemidesmosome"=c("GO:0030056","GO:0031581"),
                     ...
                     "fatty acid transporter"=c("GO:0005324","GO:0015245"),
...
#  filter(is.na(note)) %>% 
  filter(qc_pass==1) %>% 
#  filter(note!="incomplete, allele unclear") %>% 
...
p2 <- pd %>% 
  ...
  ggplot(aes(x=t_years,y=reorder(locus,tmrca_mean))) + 
    geom_boxplot() +
    facet_wrap(~go,nrow = 1) + theme(axis.text.x = element_text(angle=90)) + ylab("")
