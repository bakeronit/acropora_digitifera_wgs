...
#  ggplot(aes(x=t_years,y=reorder(locus_label,tmrca_mean))) + 
  ...
  geom_label_repel(data= pdgo %>% mutate(locus_label = as.character(dated_loci[locus])),
                   aes(x=tmrca_mean,y=locus_label,label=go,fill=pop), 
                   direction = "both",nudge_x = 5000, xlim=c(8000,NA), 
                   force=10, size = 1.5,max.overlaps = 1000,
                   label.padding = 0.15, label.size = 0.2,segment.size=0.3, segment.alpha = 1,alpha=0.8) +
...
plot_row <- plot_grid(phaps,
                      ptimeline,
                      rel_widths = c(0.2,0.8),align = "h")
...
plot_grid(plot_row,legend_b,ncol = 1,rel_heights = c(0.93,0.07))
