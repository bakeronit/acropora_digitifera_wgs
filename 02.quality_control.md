...
6.  We remove SNPs with Inbreding Coefficient &lt; -0.05. According to
...
7.  The monomorphic sites (SNPs that are non-reference in all samples)
...
**Figure:** SNP distribution for all samples, the standard normal
...
makes sense because we filtered genotypes with very few reads covered to
ensure the accuracy. There was no sample with very high missingness in
genotype, although sample `RS3_S_250` had almost 30% genotype
missingness. We decided to keep it in population structure analysis but
to remove it from analysis based on phasing (haplotype based analysis).

We noticed one single sample `BR_5_121_S125_L004` from inshore were
clustered with south offshore samples. This could be a first-generation
migrate or a tag error. To avoid the confusion, we remove that sample in
demographic history estimate and selection anlaysis.

## Prepare data for population structure analysis

We perform further filtering steps for [population genetic
analysis](04.population_structure.md).
...
-   All samples (unphased) except the mis-located sample from each
    location in **SMC++**.
